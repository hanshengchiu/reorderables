...
//    debugPrint('_requestAnimationToNextIndex _dragStartIndex:$_dragStartIndex _ghostDisplayIndex:$_ghostDisplayIndex _currentDisplayIndex:$_currentDisplayIndex _nextDisplayIndex:$_nextDisplayIndex isAcceptingNewTarget:$isAcceptingNewTarget isCompleted:${_entranceController.isCompleted}');
    if (_entranceController.isCompleted) {
      ...
      if (!isAcceptingNewTarget && _nextDisplayIndex == _currentDisplayIndex) {// && _dragStartIndex == _ghostIndex
        return;
      }
...
//      debugPrint(' _childRunIndexes:$_childRunIndexes _nextChildRunIndexes:$_nextChildRunIndexes _wrapChildRunIndexes:$_wrapChildRunIndexes');
      ...
      Widget _includeMovedAdjacentChildIfNeeded(Widget child, int childDisplayIndex) {
...
//        debugPrint(' make $checkingTargetIndex($checkingTargetDisplayIndex) disappearing around $index');
        ...
        return _buildContainerForMainAxis(children: _ghostDisplayIndex < _currentDisplayIndex ? [disappearingPreChild, child] : [child, disappearingPreChild]);
      }
      ...
      if (_currentDisplayIndex == -1 || displayIndex == _currentDisplayIndex) {
        ...
        return _buildContainerForMainAxis(children: [_includeMovedAdjacentChildIfNeeded(draggable, displayIndex)]);
      }

      bool _onWillAccept(Key toAccept, bool isPre) {
        ...
        bool willAccept = _dragging == toAccept && toAccept != toWrap.key && (_entranceController.isCompleted || !movingToAdjacentChild) && _currentDisplayIndex != nextDisplayIndex;
...
//            }
      ...
      Widget preDragTarget = DragTarget<Key>(
        builder: (BuildContext context, List<Key> acceptedCandidates, List<dynamic> rejectedCandidates) => SizedBox(),
        onWillAccept: (Key toAccept) => _onWillAccept(toAccept, true),
        onAccept: (Key accepted) {},
        onLeave: (Key leaving) {},
      );
      Widget nextDragTarget = DragTarget<Key>(
        builder: (BuildContext context, List<Key> acceptedCandidates, List<dynamic> rejectedCandidates) => SizedBox(),
        onWillAccept: (Key toAccept) => _onWillAccept(toAccept, false),
        onAccept: (Key accepted) {},
        onLeave: (Key leaving) {},
      );
...
//        debugPrint(' make $index appearing');
